...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-white text-lg font-medium">Tambah Testimoni Baru</h2>
        <p class="text-white/40 text-sm font-light">Masukkan ulasan dari klien.</p>
    </div>
            ...
            <div class="lg:col-span-2 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required class="w-full bg-[#0a1f12] border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#9acb03] focus:ring-1 focus:ring-[#9acb03] transition-all outline-none">
                    </div>
                    <div>
                        <label class="block text-white/70 text-xs font-medium mb-2">Perusahaan / Instansi</label>
                        <input type="text" name="company" value="{{ old('company') }}" class="w-full bg-[#0a1f12] border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#9acb03] focus:ring-1 focus:ring-[#9acb03] transition-all outline-none">
                    </div>
                </div>

                <div>
                    <label class="block text-white/70 text-xs font-medium mb-2">Isi Testimoni <span class="text-red-400">*</span></label>
                    <textarea name="content" rows="4" required class="w-full bg-[#0a1f12] border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#9acb03] focus:ring-1 focus:ring-[#9acb03] transition-all outline-none">{{ old('content') }}</textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-white/70 text-xs font-medium mb-2">Layanan yang Digunakan</label>
                        <input type="text" name="service_used" value="{{ old('service_used') }}" placeholder="Contoh: Jasa Pembuatan Website" class="w-full bg-[#0a1f12] border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#9acb03] transition-all outline-none">
                    </div>
                    <div>
                        <label class="block text-white/70 text-xs font-medium mb-2">Asal Kota <span class="text-white/30">(Opsional)</span></label>
                        <input type="text" name="city" value="{{ old('city') }}" class="w-full bg-[#0a1f12] border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#9acb03] transition-all outline-none">
                    </div>
                </div>
                
                <div>
                    <label class="block text-white/70 text-xs font-medium mb-2">City Key <span class="text-white/30">(Untuk ditampilkan khusus di Landing Page Kota tertentu)</span></label>
                    <select name="city_key" class="w-full bg-[#0a1f12] border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#9acb03] transition-all outline-none">
                        ...
                        <option value="{{ $key }}" {{ old('city_key') == $key ? 'selected' : '' }}>{{ $city['name'] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                        ...
                        <div class="border-2 border-dashed border-white/10 rounded-xl p-4 text-center hover:border-[#9acb03]/50 transition-colors relative cursor-pointer"
                            ...
                            <template x-if="!preview">
                                <div class="py-6">
                                    ...
                                    <span class="text-xs text-white/50 block">Klik untuk memilih foto</span>
                                </div>
                            </template>
                            <template x-if="preview">
                                <img :src="preview" class="w-24 h-24 mx-auto object-cover rounded-full border-4 border-white/10">
                            </template>
