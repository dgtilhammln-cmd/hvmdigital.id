...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        <h2 class="text-white text-lg font-medium">Daftar Layanan</h2>
        <p class="text-white/40 text-sm font-light">Kelola layanan digital dan jasa yang Anda tawarkan.</p>
    </div>
    <a href="{{ route('admin.services.create') }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-[#9acb03] text-[#053d33] font-semibold text-sm rounded-xl hover:bg-[#b8e832] transition-colors shadow-lg">
        ...
        <table class="w-full text-left text-sm text-white/70">
            <thead class="text-xs text-white/40 uppercase bg-white/5 border-b border-white/5">
                <tr>
                    ...
                    <th class="px-6 py-4 font-medium">Mulai Dari (Price)</th>
            ...
            <tbody class="divide-y divide-white/5 font-light">
                ...
                <tr class="hover:bg-white/[0.02] transition-colors group">
                    <td class="px-6 py-4">
                        ...
                        <img src="{{ get_image_url($item->featured_image) }}" alt="{{ $item->name }}" class="w-16 h-12 object-cover rounded-lg">
                        @else
                    ...
                    <td class="px-6 py-4">
                        ...
                        <div class="text-[11px] text-white/40 mt-0.5">{{ Str::limit($item->short_description, 60) }}</div>
                    </td>
                    <td class="px-6 py-4">
                        @if($item->price_start)
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.services.edit', $item) }}" class="p-2 text-white/50 hover:text-[#9acb03] hover:bg-[#9acb03]/10 rounded-lg transition-colors" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <form action="{{ route('admin.services.destroy', $item) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus layanan ini?');">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-white/40">Belum ada layanan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
