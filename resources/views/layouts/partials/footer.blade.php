<footer class="bg-[#075749] dark:bg-[#061009] border-t border-white/10 dark:border-white/5 pt-16 pb-8">
    <div class="container mx-auto px-4 lg:px-8">
        ...
        <div class="h-px mb-14 rounded-full"
            style="background:linear-gradient(90deg,transparent 0%,rgba(154,203,3,0.3) 30%,rgba(154,203,3,0.7) 60%,rgba(154,203,3,0.3) 85%,transparent 100%);">
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-14">
            ...
            <div class="lg:col-span-1">
                <div class="mb-5">
                    ...
                    @endphp
                    <img src="{{ $footerLogoUrl }}" alt="{{ setting('site_name', 'HVM Digital') }}"
                        class="h-10 w-auto brightness-0 invert" loading="lazy"
                ...
                <p class="text-white/50 text-sm font-light leading-relaxed mb-6 max-w-xs">
                    Harbour Visionary Minds - mitra One-Stop Solution Digital Marketing & IT untuk bisnis Anda tumbuh
                    dan ditemukan pelanggan.
                </p>
                <div class="flex gap-3">
                    <a href="{{ setting('instagram', '#') }}" target="_blank" rel="noopener"
                        aria-label="Instagram HVM Digital"
                    ...
                    <a href="{{ wa_link() }}" target="_blank" rel="noopener" onclick="trackWaClick('footer')"
                        aria-label="WhatsApp HVM Digital"
                ...
                <ul class="space-y-3">
                    @foreach(['Pembuatan Website', 'SEO & Optimasi', 'Google & Meta Ads', 'Social Media', 'Desain Grafis', 'Aplikasi Custom'] as $s)
                        <li><a href="{{ route('services') }}"
                                class="text-white/50 hover:text-[#9acb03] text-sm font-light transition-colors">{{ $s }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
                ...
                <ul class="space-y-3">
                    ...
                    @foreach($footerCities as $key => $name)
                        @if(isset(config('cities')[$key]))
                            <li>
                                <a href="{{ url(config('cities')[$key]['slug']) }}"
                                    class="text-white/50 hover:text-[#9acb03] text-sm font-light transition-colors flex items-center gap-2">
                    ...
                    <li>
                        <a href="{{ route('services') }}"
                            class="text-[#9acb03] text-xs font-medium hover:underline">Lihat semua kota &rarr;</a>
                    </li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-4">
                    <li class="flex gap-3 items-start">
                        ...
                        <div>
                            <p class="text-white/80 text-[11px] font-semibold mb-0.5">Surabaya <span
                                    class="text-[#9acb03] font-normal">(Pusat)</span></p>
                            <span
                                class="text-white/40 text-xs font-light leading-relaxed block">{{ setting('address_surabaya', 'Jl. Rungkut Lor VII Dalam, Surabaya, Jawa Timur') }}</span>
                        </div>
                    </li>
                    <li class="flex gap-3 items-start">
                        <svg class="w-4 h-4 text-[#9acb03] mt-0.5 shrink-0" fill="none" stroke="currentColor"
                        ...
                        <div>
                            <p class="text-white/80 text-[11px] font-semibold mb-0.5">Bekasi <span
                                    class="text-white/30 font-normal">(Jabodetabek)</span></p>
                            <span
                                class="text-white/40 text-xs font-light leading-relaxed block">{{ setting('address_bekasi', 'Kota Bekasi, Jawa Barat') }}</span>
                        </div>
                    </li>
                    <li class="flex gap-3 items-center">
                        <svg class="w-4 h-4 text-[#9acb03] shrink-0" fill="currentColor" viewBox="0 0 24 24">
                            <path
                                d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                        </svg>
                        <a href="{{ wa_link() }}" target="_blank"
                            class="text-white/50 hover:text-[#9acb03] text-sm font-light transition-colors">{{ setting('whatsapp_display', '+62851-7998-2373') }}</a>
                    </li>
                    <li class="flex gap-3 items-center">
                        <svg class="w-4 h-4 text-[#9acb03] shrink-0" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                        <a href="mailto:{{ setting('email','bisnis@hvm-digital.id') }}"
                            class="text-white/50 hover:text-[#9acb03] text-sm font-light transition-colors">{{
                            setting('email','bisnis@hvm-digital.id') }}</a>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-5">
                @foreach([['home', 'Beranda'], ['services', 'Layanan'], ['portfolio', 'Portfolio'], ['contact', 'Kontak']] as [$r, $l])
                    <a href="{{ route($r) }}"
                        class="text-white/30 hover:text-[#9acb03] text-xs font-light transition-colors">{{ $l }}</a>
                @endforeach
            </div>
        </div>
    </div>
