{{-- === CTA SECTION === --}}
@php
$ctaImage = setting('cta_section_image');
$waOffice = 'https://wa.me/'.setting('whatsapp','6285179982373').'?text='.urlencode('Halo HVM Digital, saya ingin konsultasi gratis untuk project website bisnis saya');
@endphp
<section class="py-16 md:py-20 bg-white dark:bg-[#0a1510]">
    <div class="container mx-auto px-4 lg:px-8">
        <div class="relative rounded-3xl overflow-hidden shadow-2xl max-w-6xl mx-auto"
            ...
            <div class="flex flex-col lg:flex-row items-center justify-between gap-8 p-8 md:p-10 relative z-10">
                ...
                {{-- Left: Admin-uploadable photo / Team Visual --}}
                <div class="w-full lg:w-1/4 relative flex items-center justify-center shrink-0">
                    @if($ctaImage)
                    <img src="{{ get_image_url($ctaImage) }}"
                         alt="Tim HVM Digital Siap Membantu Bisnis Anda"
                         class="max-w-full h-auto max-h-[240px] object-contain object-center hover:scale-105 transition-transform duration-500"
                         loading="lazy">
                    @else
                    {{-- Placeholder when no image uploaded --}}
                    <div class="w-full min-h-[180px] rounded-2xl flex flex-col items-center justify-center p-6 text-center bg-white/10 border border-white/20">
                        <svg class="w-12 h-12 text-white/60 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                        </svg>
                        <p class="text-white/60 text-[11px] font-light leading-snug">Upload foto CS / Tim di Admin → Settings → Foto CTA Section</p>
                    </div>
                    @endif
                </div>
                ...
                <div class="flex-1 text-center lg:text-left px-0 lg:px-4">
                    ...
                    <p class="text-sm md:text-base text-white/90 font-light leading-relaxed max-w-xl mx-auto lg:mx-0">
                        Tim HVM Digital siap mewujudkan website impian bisnis Anda. Hubungi tim representatif kami sekarang untuk sesi konsultasi gratis & penawaran terbaik!
                    </p>
                </div>

                {{-- Right: Single Office Button --}}
                <div class="flex w-full sm:w-auto shrink-0 justify-center">
                    <a href="{{ $waOffice }}" target="_blank" rel="noopener" onclick="trackWaClick('cta-office')"
