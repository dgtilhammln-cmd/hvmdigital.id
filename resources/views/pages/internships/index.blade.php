...
<section class="relative pt-40 pb-20 overflow-hidden" style="background:linear-gradient(135deg,#053d33 0%,#075749 60%,#0a6d58 100%);">
    ...
    <div class="relative container mx-auto px-4 lg:px-8 text-center max-w-3xl">
        ...
        <nav class="flex items-center justify-center gap-2 text-white/40 text-xs font-light mb-6 flex-wrap" aria-label="Breadcrumb">
            ...
            <span class="text-[#9acb03]">Internship</span>
        </nav>

        <span class="inline-block text-[#9acb03] text-xs font-semibold tracking-widest uppercase mb-4">Program Magang</span>
        ...
        <div class="flex items-center justify-center gap-6 mt-8 text-white/40 text-xs font-light">
            <span class="flex items-center gap-1.5">
                ...
                {{ $internships->count() }} posisi dibuka
            </span>
            <span class="flex items-center gap-1.5">
    ...
    <div class="container mx-auto px-4 lg:px-8 max-w-5xl">
        <div class="grid grid-cols-1 gap-6">
                ...
                <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-3">
                            <span class="bg-[#075749]/5 dark:bg-[#9acb03]/10 text-[#075749] dark:text-[#9acb03] text-xs font-semibold px-3 py-1 rounded-full">{{ $intern->division }}</span>
                            <span class="text-gray-400 dark:text-white/40 text-xs font-light flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> {{ $intern->duration }}</span>
                        </div>
                        <h2 class="text-2xl font-bold text-[#0a1f12] dark:text-white mb-2">{{ $intern->title }}</h2>
                    ...
                    <div class="mt-4 md:mt-0 shrink-0 border-t md:border-t-0 border-gray-100 dark:border-white/5 pt-6 md:pt-0">
                        @php
                            $waMessage = "Halo HVM Digital, saya tertarik melamar posisi Internship: *" . $intern->title . "*. Saya ingin mengetahui informasi lebih lanjut mengenai proses pendaftarannya.";
                            $applyLink = !empty($intern->custom_link) ? $intern->custom_link : wa_link($waMessage);
                        @endphp
                        <a href="{{ $applyLink }}" target="_blank" onclick="trackWaClick('internship_apply')" class="inline-flex items-center justify-center w-full md:w-auto gap-2 bg-gradient-to-r from-[#075749] to-[#9acb03] text-white font-semibold text-sm px-8 py-4 rounded-xl hover:scale-105 transition-all shadow-[0_4px_15px_rgba(154,203,3,0.3)]">
                            Apply Sekarang
            ...
            <div class="text-center py-20 bg-white dark:bg-[#0d1f15] rounded-3xl border border-[#075749]/10 dark:border-[#9acb03]/10">
                ...
                <p class="text-gray-500 dark:text-white/50 font-light">Saat ini kami belum membuka program internship baru. Pantau terus halaman ini untuk update selanjutnya.</p>
            </div>
            @endforelse
        </div>
    </div>
