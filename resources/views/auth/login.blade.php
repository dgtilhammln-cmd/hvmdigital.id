...
<section class="min-h-screen flex w-full">
    ...
    <div class="hidden lg:flex w-1/2 flex-col justify-between p-12 lg:p-16 relative">
        ...
        <div class="my-auto pt-16">
            <h1 class="text-5xl lg:text-6xl font-bold text-fg tracking-tight mb-6 leading-tight">
                Kelola Website <br>
                <span class="text-[#9acb03]">Bisnis Anda!</span>
            </h1>
            <p class="text-muted text-lg font-light max-w-md leading-relaxed">
                Masuk ke dashboard untuk mengelola konten, pesanan, dan memantau perkembangan website UMKM Anda.
            </p>
        </div>
    ...
    {{-- Right Side (Form) --}}
    <div class="w-full lg:w-1/2 flex flex-col justify-center p-8 sm:p-12 lg:p-16 relative bg-white/5 dark:bg-black/20 backdrop-blur-xl border-l border-white/5 shadow-2xl">
        ...
        <div class="w-full max-w-md mx-auto">
            <div class="mb-10 text-center lg:text-left">
                <h2 class="text-3xl font-bold text-fg mb-2">Masuk ke akun.</h2>
                <p class="text-muted text-sm font-light">Gunakan email dan password HVM Digital Anda.</p>
            </div>

            @if (session('success'))
            <div class="mb-6 p-4 rounded-xl bg-green-500/10 text-green-500 text-sm font-medium border border-green-500/20">
                {{ session('success') }}
            </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div class="relative group">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus
                        class="w-full bg-transparent border-0 border-b-2 border-theme focus:border-[#9acb03] focus:ring-0 px-0 py-2.5 text-fg font-medium text-base transition-colors"
                ...
                <div class="relative group">
                    <div class="flex items-center justify-between mb-1">
                        <label for="password" class="block text-xs font-semibold text-muted uppercase tracking-wider">Password</label>
                        <a href="#" class="text-xs text-[#9acb03] hover:underline">Lupa password?</a>
                    </div>
                    <div class="relative w-full flex items-center" x-data="{ show: false }">
                        <input :type="show ? 'text' : 'password'" name="password" id="password" required
                            class="w-full bg-transparent border-0 border-b-2 border-theme focus:border-[#9acb03] focus:ring-0 px-0 py-2.5 pr-10 text-fg font-medium text-base transition-colors"
            ...
            <div class="mt-8 text-center text-sm text-muted font-light">
                <p>Belum punya akun? <a href="{{ route('register') }}" class="font-bold text-fg hover:text-[#9acb03] transition-colors">Daftar sekarang</a></p>
                <p class="pt-4 text-xs opacity-70">Admin? <a href="{{ route('admin.login') }}" class="hover:text-white underline">Klik di sini</a></p>
            </div>
        </div>
    </div>
</section>
